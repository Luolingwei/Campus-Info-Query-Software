...
namespace Whu038.Classes
{
    class OperateMap
    {
        ...
        /// <returns></returns>
        public IFeatureLayer GetFeatLyrByName1(ILayer pLayer, string sFeatLyrName)
        {
            ILayer pLyr = null;
            ...
            else
            {
                ...
                {
                    pLyr = pComLyr.get_Layer(i);
                    GetFeatLyrByName1(pLyr, sFeatLyrName);
                }
            }
            return pFeatureLyr;
        }
        ...
        public IFeatureLayer GetFeatLyrByName(IMap pMap, string sFeatLyrName)
        {
            ...
            {
                pLayer = pMap.get_Layer(i);
                pFeatLyr = GetFeatLyrByName1(pLayer, sFeatLyrName);
            ...
            try
            {
                ...
                for (int i = 0; i < pMap.LayerCount; i++)
                {
                    pLayer1 = pMap.get_Layer(i);
                    pFeatLyr = GetFeatLyrByName1(pLayer1, sFeatLyrName);
                    if (pFeatLyr != null) break;
                }
            }
            catch (Exception ex)
